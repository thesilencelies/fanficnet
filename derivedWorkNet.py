#forms source/derived pairs for training with
#the way we'll train with these is parse one source body, then many derived works for that source body
...
#edited for use with data generation by Stephen Lilico


"""Classes to load textual data in source/derived pairs."""
...
#this simple edit and form design won't work due to the paired nature of the data
#we have to keep the data together in a manner better fitting for the purpose we're using it
#on output we need to provide different sorts of batches
#so what we need are two lists of text objects - that should be doable
#to load the data we go through the test and the training folder and we load the files with
#the same id token. So each source in the training folder has names of form source_i.txt
#and each set of derived text has name derived_i.txt
#we only keep datapoints where we were able to successfully load a source and a derived with the
#same id token



#we need to deliminate between new narratives in the derived body
#we'll use a special character | which we'll replace in the text with /
#to tell us the end (this is done in preprocessing)
  
class DerivedTextDataSet(snt.AbstractModule):
  """pairs of sequence data that are source and derived text."""
  ...
  def __init__(self, num_steps=1, batch_size=1,
               ...
               name="Fanfic_Data_Set"):
    """Initializes a DerivedText sequence data object.
...
#this all has to be redone TODO
    ...
    self._source_data_source = TokenDataSource(
        data_file=self._source_data_file,
        vocab_data_file=self._source_vocab_file)
...
#we have to be able to return both batches over the source set for the first stage of training
#then the whole of the source set as one tensor, along with a slice of the derived set for the second pass
